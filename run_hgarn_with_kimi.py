...
def main():
    print("=" * 80)
    print("🔥 HGARN v1 测试 - 层次化门控注意力残差网络 (使用 Kimi API)")
    ...
    agent = HGARMultiAgent(
        ...
        parallel_execution=False,  # Kimi API不支持并行请求，串行执行
        max_parallel=1,
    ...
    # 保存结果到文件
    output_file = os.path.join(os.path.dirname(__file__), f"test_result_hgarn_kimi_{int(time.time())}.md")
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("# HGARN v1 层次化门控注意力残差网络 - Kimi API 测试结果\n\n")
